...
const Search = () => {
    const [searchErrMsg, setSearchErrMsg] = useState('¿Qué estas buscando?');
    ...
    const searchHandler = async (productName) => { // Fetch product by searched name
        try {
            const res = await axios.post('https://desi-ecommerce-backend.onrender.com/stock',
            ...
            if (Array.isArray(res.data.data) && res.data.data.length > 0) {
                ...
                setTimeout(() => { setSearchErrMsg('¿Qué estas buscando?'); }, 3000);
